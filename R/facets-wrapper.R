readSnpMatrix <- function(filename, skip=0L, err.thresh=Inf, del.thresh=Inf, perl.pileup=FALSE, MandUnormal=FALSE, spanT=0.2, spanA=0.2, spanX=0.2, gbuild="hg19") {
  ...
  if (perl.pileup) {
    ...
    # depth of unmatched normal if both matched and unmatched are counted
    ##if (MandUnormal) rcmat$UMN.DP <- pileup$File3R[ii] + pileup$File3A[ii]
    if (MandUnormal){

############################################################################################
    #add function for procUnormal()?? that computes GC, finds bestnorm auto, X and makes a final count matrix to merge with rcmat??
...
### change this to calc het snps of X from matched normal,
### and then choose normals from same sex for chrX normalization

      noiseX <- do.call('rbind',list(apply(unorms.dp.lowess.X,2,function(column){
