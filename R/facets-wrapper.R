readSnpMatrix <- function(pileupfilename, skip=0L, err.thresh=Inf, del.thresh=Inf, 
  ...
  if (perl.pileup) {
    ...
    if (MandUnormal) {
      tumor.loess <- MakeLoessObject(tumor.pileup)
...
PreProcSnpPileup <- function(filename, err.thresh=Inf, del.thresh=Inf,
  ...
  #' @param filename (string) File path to snp-pileup generated pileup file.
...
FindBestNormalParameters <- function(TumorLoess, TumorPileup,
  ...
  #' @param TumorLoess A facets2n generated TumorLoess matrix with header and span values in the first row.
  #' @param TumorPileup A snp-pileup generated pileup data frame with sample columns that match with the TumorLoess object.
  #' @param ReferenceLoess A ReferenceLoess matrix with a header and span values in the first row.
  #' @param ReferencePileupFile A snp-pileup generated pileup data frame with sample columns that match with the ReferenceLoess object.
  #' @param MinOverlap A numeric between 0 and 1 that denotes the fraction overlap of loci between TumoLoess and the optional ReferenceLoess
  ...
  noiseAuto <- do.call('rbind',list(apply(combined.loess[-x.idx,-c(1,3), drop=F],2,function(column){
    lr = log2(as.numeric(combined.loess[-x.idx,3])) - log2(as.numeric(column))
  ...
  return(list("rcmat"=rcmat,
              "spanT"=as.numeric(combined.span[3]), # Assume that tumor data is always in the third column as expected
...
######################################################################
MakeLoessObject <- function(pileup, write.loess=FALSE, outfilepath="./loess.txt") {
  ...
  #' @param pileup A data franme of snp-pileup generated depth.
